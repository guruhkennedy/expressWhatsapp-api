...
app.get("/grup", (req, res) => {
  res.sendFile("./view/page.html", {
    root: __dirname,
  });
});

app.get("/scan", (req, res) => {
    res.sendFile("./view/scan.html", {
      root: __dirname,
    });
  });
        ...
        if(type === "notify"){
            if(!messages[0].key.fromMe) {              
                ...
                if(!messages[0].key.fromMe && pesanMasuk === "ping"){
                    ...
                    await sock.sendMessage(noWa, {text: "im bot"},{quoted: messages[0] });
                }
			}		
		}		
    });
    ...
    switch (data) {
        case "qr":
            qrcode.toDataURL(qr, (err, url) => {
                soket?.emit("qr", url);
                soket?.emit("log", "Silahkan Scan QR Code ini!");
            });
            break;
        case "connected":
            soket?.emit("log", "Status api : Berhasil terhubung!");
            break;
        case "qrscanned":
            soket?.emit("log", "QR Code Berhasil discan!");
            break;
        case "loading":
            soket?.emit("log", "Tunggu Sebentar");
            break;
        default:
            break;
    }
...
app.post("/kirim-user", async (req, res) =>{
    //console.log(req);
...
// send group message
app.post("/kirim-grup", async (req, res) =>{
    //console.log(req);
    const pesankirim = req.body.message;
